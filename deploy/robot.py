...
class MyRobot(wpilib.TimedRobot):
    def robotInit(self):
        ...
        # DO NOT CHANGE THESE ANYMORE UNLESS OTHERWISE DIRECTED TO (VERY UNLIKELY, WE CAN CHANGE DURING THE WEEK PRECEEDING ALLENTOWN)
        ...
        self.drive_speed = 0.7                 #   Robot driving speed                                    (SmartDashboard)
        self.arm_speed = 0.2                   #   Robot arm speed                                         (SmartDashboard)
        self.intake_speed = 0.3                 #   Robot intake speed                                      (SmartDashboard)
        self.high_shot_speed_d = 0.8            #   Maximum shooter speed                                   (SmartDashboard)
        self.low_shot_speed_d = 0.4              #   Minimum shooter speed                                  (SmartDashboard)
        self.climber_speed = 0.45               #   Robot climber speed for switching climber position      (Hardcode)
        ...
        # Setup: Robot Control
        self.controller = wpilib.PS5Controller(0)    # Connect to a joystick. Specify: PS4Controller, PS5Controller
        ...
        wpilib.SmartDashboard.putNumber("Maximum Drive Speed", (self.drive_speed))
        wpilib.SmartDashboard.putNumber("Maximum Arm Power", (self.arm_speed))
    ...
    def teleopPeriodic(self):
        # the input of the sticks are negated because when either stick is up, they return -1.
        self.robot.tankDrive(-self.controller.getRawAxis(1) * self.robot_speed, -self.controller.getRawAxis(5) * self.robot_speed)
        ...
        if not(self.intake_active):
            if self.controller.getR2Button():
                self.shootRing(self.high_shot_speed_d, self.shot_delay)

            elif self.controller.getR1Button():
                self.shootRing(self.low_shot_speed_d, self.shot_delay)
    ...
    def moveArm(self, direction):  
        ...
        # if abs(self.mean_encoder_position) <= self.low_arm_encoder_limit:
            ...
            # if direction < 0 and not(self.limit_switch_high_arm.get()):
